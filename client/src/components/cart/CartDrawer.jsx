      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={`
              fixed z-[100] flex flex-col bg-[#09090b] border-l border-white/10 shadow-2xl
              /* Mobile: full screen */
              inset-0
              /* Desktop: right panel */
              sm:inset-y-0 sm:left-auto sm:right-0 sm:w-full sm:max-w-md
            `}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 sm:p-6 border-b border-white/10 shrink-0">
              <h2 className="text-base sm:text-lg font-heading tracking-widest text-white">
              ...
              <button
                ...
                className="flex items-center justify-center w-10 h-10 rounded-lg border border-white/15 text-white/60 hover:text-white hover:border-white/40 transition-colors"
              >
            ...
            {/* Items */}
            <div className={`bg-black px-5 py-5 sm:p-6 space-y-6 ${cart.length > 3 ? 'flex-1 overflow-y-auto' : cart.length === 0 ? 'flex-1' : ''}`}>
              {cart.length === 0 ? (
                ...
                <div className="space-y-6">
                  {cart.map((item) => (
                    ...
                    >
                      <div className="w-20 h-28 sm:w-24 sm:h-32 bg-white/5 rounded-sm overflow-hidden flex-shrink-0">
                        {item.image ? (
                          <img
                            src={item.image}
                            alt={item.name}
            ...
            {/* Footer */}
            {cart.length > 0 && (
              <div className="px-5 py-5 sm:p-6 border-t border-white/10 bg-[#09090b] shrink-0">
                ...
                <Link
                  to="/checkout"
                  onClick={() => setIsOpen(false)}
                  className="w-full block bg-white text-black py-4 text-sm tracking-[0.2em] uppercase font-bold hover:bg-[rgb(0,255,255)] transition-colors duration-300 text-center"
                >
                  Iniciar Compra
                </Link>
