        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-12 lg:px-16 py-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link
              to="/"
              onClick={close}
              className="shrink-0 text-white font-heading text-[1.7rem] md:text-2xl font-semibold tracking-[0.04em] uppercase"
            >
              SOLUTION
            </Link>
            ...
            <div className="hidden md:flex items-center gap-6 lg:gap-8">
              <Link to="/" className="nav-link-underline text-white text-sm uppercase tracking-[0.06em] hover:text-white/80 transition-colors">INICIO</Link>
              <Link to="/tienda" className="nav-link-underline text-white text-sm uppercase tracking-[0.06em] hover:text-white/80 transition-colors">TIENDA</Link>
              {!loading && (
                <>
                  <Link to={wholesaleTo} className="nav-link-underline text-white text-sm uppercase tracking-[0.06em] hover:text-white/80 transition-colors">MAYORISTA</Link>
                  {user && isAdmin && (
                    <Link to="/admin" className="nav-link-underline text-white text-sm uppercase tracking-[0.06em] hover:text-white/80 transition-colors">ADMIN</Link>
                  )}
                  {user && (
                    <div className="relative">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-white/80 hover:text-white text-sm uppercase tracking-wide transition-colors"
                        aria-expanded={accountOpen}
                      >
                        <span className="max-w-[160px] truncate">{desktopAccountLabel}</span>
                        <span className="text-white/50 text-[10px]">▼</span>
                      </button>
                      {accountOpen && (
                        <>
                          <div className="fixed inset-0 z-0" aria-hidden onClick={() => setAccountOpen(false)} />
                          <div className="absolute right-0 top-full mt-2 py-2 w-52 bg-black/95 backdrop-blur-xl border border-white/15 rounded-xl z-10 shadow-xl">
                            <button
                              type="button"
                              onClick={handleLogout}
                              className="block w-full text-left px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/10 transition-colors"
                            >
                              Cerrar sesión
                            </button>
                          </div>
...
function MobileLink({ to, onClick, children, subtle = false }) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className={`block px-2 py-3 text-sm uppercase tracking-widest transition-colors ${subtle
